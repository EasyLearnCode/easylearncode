...
(function () {
    angular.module("easylearncode.core", ["ngResource", "services", "controllers", "directives", "oldFilters", "oldMeta", "ngRoute"]);
...
angular.module("easylearncode.contest")
    .config(['$routeProvider', function($routeProvider){
        $routeProvider
            .when('/',
            {
                templateUrl:'/templates/angular/contest/contest.html',
    ...
    ["$scope", "api", "$http", "csrf_token", "channelToken", "$location",'$timeout', '$window',
                        ...
                        if (lang.active) {
                            if (lang.source) {
                                if (type == 'submit') {
                                    ...
                                    bootbox.prompt("Nhập tên file", function (filename) {
                                        filename = filename || "script";
                                        $scope.compiling = true;
                                        $http.post('/api/contest/submit', {"weekly_quiz_level_key": $scope.current_level.Id, "source": lang.source,
                                            "_csrf_token": csrf_token, "lang": lang.lang, "type": type, 'filename': filename}).success(function (data) {
        ...
        .controller('ContestResultCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
            $scope.selectedDate = new Date();
            $scope.getWeek = function () {
                $scope.weeks = new Array();
                var date_start = $scope.selectedDate.moveToFirstDayOfMonth().toISOString();
                var date_finish = $scope.selectedDate.moveToLastDayOfMonth().toISOString();
                $http.get('/api/quizs?recurse=true&filter=start_date>=' + date_start + '&filter=start_date<=' + date_finish).success(function (data) {
                    $scope.details = {};
                    $scope.currentWeek = {};
                    $scope.weeks = data;
                    $scope.getResult($scope.weeks[0]);
                });
            }

            $scope.getResult = function (week) {
                $scope.currentWeek = week;
            }
            $scope.getDetail = function (key) {
                $http.get('/api/contest/info?recurse=True&Id=' + $scope.currentWeek.Id + "&user_id=" + key).success(function (data) {
...
angular.module("easylearncode.learn")
    .config(['$routeProvider', function ($routeProvider) {
        $routeProvider
            .when('/', {
                templateUrl: '/templates/angular/learn/learn.html',
    ...
    .controller('LearnCtrl', ['$scope', '$http', '$location', '$sce', '$compile', '$window', 'api', '$timeout', 'localStorageService', 'csrf_token', "$q", '$rootScope', 'VG_EVENTS','$modal', function ($scope, $http, $location, $sce, $compile, $window, api, $timeout, localStorageService, csrf_token, $q, $rootScope, VG_EVENTS, $modal) {
        ...
        $scope.current_description = 'Easylearncode, Hệ thống hỗ trợ dạy học lập trình tương tác đầu tiên tại Việt Nam';
        ...
        $scope.runCodeInHackerEarth = function () {
            $http.post('/api/runcode', {"lang": $scope.lang.lang, "source": $scope.code, "_csrf_token": csrf_token})
        ...
        $scope.onQuizSubmit = function (data) {
            ...
            if (data.type == "Quiz") {
                quizs = _.where($scope.lecture.quizs, {Id: data.id});
                result = false;
                description = "";
                if (quizs.length == 0) {
                    ...
                    answer = _.where(quizs[0].answers, {Id: data.answer})
                ...
                tests = _.where($scope.lecture.tests, {Id: data.id});
        ...
        $scope.onCompleteVideo = function () {
            ...
            var scores = 0;
            _.each($scope.config.plugins.quiz.data, function (elm) {
            ...
            var cur_index = $scope.getCurrentLectureIndex();
            if (score * 100 / scores > 70) {
                ...
                $http.post('/api/users/me/passedLecture', {lecture_id: $scope.lecture.Id}).success(function (data) {
                    if (cur_index == $scope.lectures.length - 1) {
                        ...
                        $http.post('/api/users/me/currentLecture', {lecture_id: $scope.lectures[cur_index + 1].Id});
                        $scope.showAlert = true;
                        $scope.alert = {
                            type: 'info',
        ...
        $scope.onUpdateTime = function (currentTime, totalTime) {
            ...
            angular.forEach($scope.lecture.codes, function (code) {
        ...
        $scope.loadLecture = function () {
            ...
            $scope.config.plugins.quiz.data = $scope.lecture.quizs;
            angular.forEach($scope.lecture.tests, function (test) {
                $scope.config.plugins.quiz.data.push(test);
            });
            $scope.rate = $scope.lecture.rate;
            if($scope.lecture._can_rate){
                $scope.isReadonly = false;
            }else{
                $scope.isReadonly = true;
            }
            $http.post("/api/users/me/currentLecture", {lecture_id: $scope.lecture.Id});
        }
        ...
        $window.learnAgain = function(){
            $window.location.reload();
        }



        //-------------begin jsrepl callback-----------------
        ...
        $timeout(function(){
            ...
            api.Model.get({type: 'lectures', id: $location.search()['lecture_id'], extras:'lecture_extras'}, function (lecture) {
                $scope.loadingComplete();
                $scope.lecture = lecture;
                $scope.lang = _.find($scope.langs,function(lang){
                    return (lang.name.toLowerCase() == $scope.lecture.lecture_language.toLowerCase() || lang.lang.toLowerCase() == $scope.lecture.lecture_language.toLowerCase());
                });
                if($scope.lang.jsrepl){
                    ...
                    angular.forEach(course.lessons, function (lesson) {
                        angular.forEach(lesson.lectures, function (lecture) {
                            if (lecture.Id == $scope.lecture.Id) {
                                lecture._is_current_lecture = true;
                            } else {
